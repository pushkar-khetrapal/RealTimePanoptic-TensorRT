...
@torch.jit.script
def get_process_done(locations, box_cls, box_regression, centerness, levelness_logits, semantic_logits, image_sizes):
    ...
    for i, (l, o, b, c) in enumerate(zip(locations[:-1], box_cls, box_regression, centerness)):
        sampled_boxes.append(PanopticFromDenseBox.forward_for_single_feature_map(l, o, b, c, image_sizes))
    ...
    try:
        ...
        boxlists = PanopticFromDenseBox.select_over_all_levels(boxlists)
    ...
    bounding_box_feature_map = PanopticFromDenseBox.generate_box_feature_map(levelness_locations, box_regression, levelness_logits)
    ...
    semantic_probability = semantic_probability[:, :, (11, 18)[0]:]
    boxlists = PanopticFromDenseBox.mask_reconstruction(
        boxlists=boxlists,
...
@torch.jit.script
def get_forward_for_single_feature_map(locations, box_cls, box_regression, centerness, image_sizes):
    ...
    candidate_inds = box_cls > 0.05 #pre_nms_thresh
    ...
    pre_nms_top_n = pre_nms_top_n.clamp(max=1000)  #pre_nms_top_n
...
@torch.jit.script
def get_generate_box_feature_map(location, box_regression, levelness_logits):
...
@torch.jit.script
def get_mask_reconstruction(boxlists, box_feature_map, semantic_prob, box_feature_map_location, h_map, w_map):
...
@toch.jit.script
def get_select_over_all_levels(boxlists):
    ...
    for i in range(num_images):
        ...
        for j in range(1, 19):
            ...
            if len(inds) > 0:
                ...
                boxlist_for_class = boxlist_nms(boxlist_for_class, 0.6, score_field="scores")
        ...
        if number_of_detections > 100 > 0:
            ...
            image_thresh, _ = torch.kthvalue(cls_scores.cpu(), number_of_detections - 100 + 1)
...
class PanopticFromDenseBox:
    ...
    def process(
        ...
        return get_process_done(locations, box_cls, box_regression, centerness, levelness_logits, semantic_logits, image_sizes)


    def forward_for_single_feature_map(self, locations, box_cls, box_regression, centerness, image_sizes):

        return get_forward_for_single_feature_map(locations, box_cls, box_regression, centerness, image_sizes)

    def generate_box_feature_map(self, location, box_regression, levelness_logits):

        return get_generate_box_feature_map(location, box_regression, levelness_logits)


    def mask_reconstruction(self, boxlists, box_feature_map, semantic_prob, box_feature_map_location, h_map, w_map):

        return get_mask_reconstruction(boxlists, box_feature_map, semantic_prob, box_feature_map_location, h_map, w_map)
